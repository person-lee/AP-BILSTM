...
#----------------------------- cal attention -------------------------------
#input_q, input_a (batch_size, seq_len, rnn_size)
def cal_attention(input_q, input_a, U, batch_size):
    G = tf.batch_matmul(tf.batch_matmul(input_q, tf.tile(tf.expand_dims(U, 0), [batch_size, 1, 1])), input_a, adj_y=True)
